...
export class GenerarPdfComponent implements OnInit {
  ...
  async cargarSecciones() {
    const tipos = ['idea', 'objetivo', 'costos', 'mano-obra', 'costos-indirectos', 'pon-en-marcha'];
  ...
  generarPDF() {
    ...
    doc.setFontSize(12);
    // IDEA
    doc.text('🧠 Parte 1: Idea de Negocio', 10, y); y += 10;
    ...
    // OBJETIVO
    y += 10;
    doc.text('🎯 Parte 2: Objetivo', 10, y); y += 10;
    ...
    y += 10;
    doc.text('💰 Parte 3.1: Costos - Materia Prima', 10, y); y += 10;
    ...
    if (costos) {
      ...
      doc.text('Ingredientes:', 10, y); y += 10;
    ...
    // MANO DE OBRA
    y += 10;
    doc.text('👷 Parte 3.2: Mano de Obra', 10, y); y += 10;
    ...
    // COSTOS INDIRECTOS
    y += 10;
    doc.text('🏭 Parte 3.3: Costos Indirectos', 10, y); y += 10;
    const ci = this.secciones['costos-indirectos'];
    if (ci && ci.indirectos) {
      ci.indirectos.forEach((ind: any) => {
        addText(`- ${ind.descripcion}`, `Costo mensual: ${ind.costoMensual} Bs`);
      });
    }

    // PON EN MARCHA
    y += 10;
    doc.text('🚀 Parte 4: Pon en Marcha', 10, y); y += 10;
